...
async def ensure_platform_service_subgroup(
    realm_name: str, group: t.Dict[str, t.Any], service_name: str
):
    """
...
async def prune_platform_service_subgroups(
    realm_name: str, platform: api.Platform, group: t.Dict[str, t.Any]
):
    """
...
async def ensure_platform_service_client(
    ...
    base_url = "{scheme}://{fqdn}".format(
        scheme=settings.keycloak.zenith_redirect_uri_scheme, fqdn=service.fqdn
    )
    next_client.update(
        {
            "clientId": client_id,
...
async def prune_platform_service_clients(
    realm_name: str, platform: api.Platform, all: bool = False
):
    """
