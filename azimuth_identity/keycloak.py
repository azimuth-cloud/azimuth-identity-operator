...
# The client must be initialised inside the event loop for auth lock to work correctly
kc_client = None
...
async def ensure_user_profile_config(realm_name: str):
    ...
    # On older versions, this looks for a user with ID "profile", which will never exist
...
async def _ensure_group(realm_name: str, group_name: str):
    ...
    try:
        ...
        response = await kc_client.post(
            f"/{realm_name}/groups", json={"name": group_name}
        )
...
async def ensure_admins_group(realm_name: str):
    ...
    for client_name, roles in settings.keycloak.admins_group_client_roles.items():
        ...
        if missing_roles:
            ...
            # We need to get the ID for each role,
            # so load the available client roles and index them
            response = await kc_client.get(
...
async def ensure_identity_provider(realm: api.Realm, realm_name, dex_client):
    ...
    idp_url = (
        ...
        f"{settings.dex.keycloak_client_alias}"
    )
...
async def _ensure_idp_first_login_flow(realm: api.Realm, realm_name: str):
    ...
    try:
        ...
        if exc.response.status_code != 404:
            raise
        # If the executions don't exist, we need to create the flow
        # by copying the source flow
        LOGGER.info(
...
async def prune_platform_service_clients(
    realm_name: str, platform: api.Platform, prune_all: bool = False
):
    """
    Prunes clients for platform services.

    If prune_all is True, all clients for the platform are pruned. If False,
    only clients for unrecognised services are pruned.
    """
    ...
    for client in response.json():
        ...
        if not prune_all and service_name in platform.spec.zenith_services:
            continue
