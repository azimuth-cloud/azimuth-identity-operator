...
class OIDCClientSpec(schema.BaseModel):
    ...
    client_id: schema.Optional[schema.constr(min_length=1)] = Field(
        None, description="The client ID to use. If not given, the name is used."
    )
    public: bool = Field(
        False,
        description=(
    ...
    grant_types: list[OIDCClientGrantType] = Field(
        default_factory=list, description="The grant types to allow for the client."
    )
    redirect_uris: list[schema.AnyHttpUrl] = Field(
        default_factory=list, description="The redirect URIs for the client."
    )
    credentials_secret_name: schema.Optional[schema.constr(min_length=1)] = Field(
        None,
        description=(
            "The name of the secret into which client credentials should be written."
        ),
    )
...
class OIDCClientStatus(schema.BaseModel, extra="allow"):
    ...
    phase: OIDCClientPhase = Field(
        OIDCClientPhase.UNKNOWN.value, description="The phase of the OIDC client."
    )
    issuer_url: schema.Optional[schema.constr(min_length=1)] = Field(
        None, description="The issuer URL for the OIDC client."
    )
    client_id: schema.Optional[schema.constr(min_length=1)] = Field(
        None, description="The client ID of the OIDC client."
    )
    credentials_secret_name: schema.Optional[schema.constr(min_length=1)] = Field(
        None,
