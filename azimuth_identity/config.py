...
class DexConfig(Section):
    ...
    #: By default, this points to a local pull of the chart that is baked into the image
    chart_name: constr(min_length=1) = "/charts/dex"
    chart_repo: AnyHttpUrl | None = None
    chart_version: constr(min_length=1) | None = None
...
class KeycloakConfig(Section):
    ...
    zenith_discovery_secret_name_template: constr(min_length=1) = (
        "oidc-discovery-{subdomain}"
    )
    ...
    admins_group_client_roles: dict[str, list[str]] = Field(
...
class HelmClientConfiguration(Section):
    ...
    default_timeout: int | constr(min_length=1) = "5m"
